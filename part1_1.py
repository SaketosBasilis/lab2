...
class RBFNetwork:
    ...
    def __init__(self, num_centers, X, sigma=.9, hta = 0.05):
...
plt.title('RBF Network Approximation sin(2x)')
...
for hidden_size in hidden:
    ...
    plt.plot(X_test, Y_test, label='RBF Network Approximation hidden_size :'+str(hidden_size), linewidth=2)
    plt.legend()
    plt.grid(True)
    plt.draw()
